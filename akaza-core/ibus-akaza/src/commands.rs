use crate::{update_preedit_text_before_henkan, AkazaContext, _make_preedit_word};
...
impl Commands {
    ...
    pub(crate) fn get_map() -> HashMap<&'static str, IbusAkazaCommand> {
        HashMap::from([
            (
                "commit_preedit",
                Commands::commit_preedit as IbusAkazaCommand,
            ),
            (
                "erase_character_before_cursor",
                Commands::erase_character_before_cursor as IbusAkazaCommand,
            ),
        ])
    }
