...
┌─────────────────────────────────┐
│  フロントエンド (鍵の管理)        │
│  ibus-akaza: salt ファイル       │
│  mac-akaza:  Keychain (将来)    │
├─────────────────────────────────┤
│  libakaza (暗号化・復号ロジック)   │
│  鍵は外部から受け取る             │
└─────────────────────────────────┘
...
**libakaza** は暗号化・復号のロジックとバイナリ形式の読み書きを担当する。鍵と salt は引数として外部から受け取り、鍵の保管場所については関知しない。

**各フロントエンド** が鍵の生成・保管を担当する:

- **ibus-akaza** (Linux): salt ファイルを `~/.local/share/akaza/` に `0600` パーミッションで保存
- **mac-akaza** (macOS): 将来的に Keychain への移行が可能。初期実装では salt ファイル方式
...
1. 初回起動時にランダムな鍵と salt を生成し、パスワードファイルに保存する
...
```bash
# ユーザー統計データの内容を復号して表示
akaza-data dump-user-stats --key-file ~/.local/share/akaza/key.bin
```
